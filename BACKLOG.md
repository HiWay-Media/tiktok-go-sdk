...
- [ ] **TT-34 — `gofmt` e gate di formato**: 14 file su 21 non sono formattati (indentazione mista tab/spazi in `constants.go`, `model_content.go`, `request_content.go`, `tiktok.go`, …). Applicare `gofmt -w` in un commit dedicato e aggiungere `gofmt -l` come gate CI, così non ricapita.
- [ ] **TT-35 — Igiene della CI**: `go-test.yml` esegue `go mod tidy` prima di buildare — in CI maschera un `go.mod`/`go.sum` incoerente invece di farlo fallire (va sostituito con `go mod download` + `go mod verify`); inoltre gira su **ogni** push esponendo i secret `CLIENT_KEY`/`CLIENT_SECRET` a job che non ne hanno bisogno (li usa solo un test legacy: vedi TT-22). `dependabot.yml` ha una entry `gomod` per la directory `/tests`, **che non esiste** (il repo ha `/test` e un solo modulo): Dependabot la segnala come errore di configurazione.
